    ...
    public class Room_TypeClient
    {
        ...
        public IEnumerable<List_Rooms> searchRoom_Type(room_Type_DescriptionViewModel model)
        {
            ...
            HttpResponseMessage response = client.GetAsync(
                "Room_Type?room_Type_Description=" + a).Result;
        ...
        public IEnumerable<Room_Type> getRoom_Type()
        {
            HttpClient client = new HttpClient();
            client.BaseAddress = new Uri(Base_URL);
            client.DefaultRequestHeaders.Accept.Add(
                new System.Net.Http.Headers.MediaTypeWithQualityHeaderValue("application/json"));
            HttpResponseMessage response = client.GetAsync("Room_Type").Result;
            if(response.IsSuccessStatusCode)
            {
                return response.Content.ReadAsAsync<IEnumerable<Room_Type>>().Result;
            }
            return null;
        }
    }
